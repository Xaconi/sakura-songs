...
const tracksWithUrls: Track[] = globalPlaylist.map((track) => ({
  ...track,
  src: getAudioUrl(track.filename),
}));

const scenePlaylists: Record<string, string[] | null> = {
  day: null,
  sunset: null,
  night: null,
};

function getSceneTracks(sceneId: string): Track[] {
...
const sceneTracksCache: Record<string, Track[]> = {
...
export function getScenePlaylist(sceneId: string): Track[] {
